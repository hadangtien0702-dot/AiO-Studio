...
| 7 | Auto Guideline Frame | **0.3.1** | `com.aiostudio.guideframe` · 8096 | ✅ Anh test ĐẠT 21/09 (0.3.0) · 🟡 **0.3.1 cài máy công ty 25/09, chờ anh bấm trên bài thật** | Safe zone 10 nền tảng / 53 vùng; đặt 0,74s / gỡ 0,13s trên sequence 4K 306 clip. **25/09:** anh bấm trên timeline 11 track đầy → "No empty video track" (*"add không được em à"*) → 0.3.1 tự thêm track video lên trên cùng (QE `addTracks(1,n,0,0)`, đo thật 298 ms, 0 track tiếng thêm nhầm) + tìm track trống theo vùng In/Out thay vì cả sequence. **25/09 chiều:** bài test của Claude làm mất guide anh vừa đặt trên "Tập 2" → lộ 2 lỗi có sẵn từ 06/08 (gỡ guide ở sequence B xoá cả bin → guide ở A mất; panel quét xoá PNG trên đĩa) → đã sửa gốc, bản cài 15:57 chưa đo lại trọn bài test (sổ lỗi tái diễn trong `CLAUDE.md` panel). Không build. Chưa có bộ cài |
...
| **24/09** | **Shot & Save: mã bản quyền QUÁ 1 NĂM vẫn kích hoạt được — dùng mãi, chỉ hết bản cập nhật (phương án A)** | Đọc mã nguồn Polar: `activate` từ chối mã hết hạn (403 "License key has expired.") → khách mua năm trước cài lại máy sẽ bị khoá, trái luật "không gia hạn thì app vẫn chạy". Anh chọn A trong bảng hỏi (B = bắt gia hạn $2 mới cài máy mới). Đổi lại: mã hết hạn không còn giới hạn 2 máy. Hằng `NHAN_MA_HET_HAN` trong `src/banquyen.js` | ✅ |
