...
| 13 | **Video Download** | **0.2.1** | `com.aiostudio.videodownload` · **8098** | 🟢 Đã cài máy công ty 21/09 | Dán link → tải (yt-dlp + QuickJS 2 MB + FFmpeg LGPL) → tự vào bin. 21/09: giao diện mới (hướng A anh chốt), sửa "Mở thư mục" (gốc: explorer bị `windowsHide` → 9 cửa sổ ẩn 1,67 GB), "Đã vào project" thành nhãn sống theo project đang mở, thư mục mặc định `<project>\AiO Studio Download`. Đo trên Premiere thật bằng project thử riêng. 21/09 14:51 anh tự test: *"thấy ổn"*, không báo lỗi. Chưa dùng trên bài dựng thật; chưa đóng gói bộ cài. Chi tiết: `CLAUDE.md` + `PROGRESS.md` trong thư mục nó |
...
| **Panel tổng "AiO Studio" (bệ phóng)** — chờ anh chọn giao diện A (lưới thẻ) · B (danh sách theo công đoạn) · C (thanh icon dọc), mỗi hướng vẽ ở 380/300px (C: 72/300px) bằng dữ liệu thật 12 tool: https://claude.ai/artifact/GmdDjRe5oRJuNBr3uAbubN | WELCOME → panel tổng | Chọn xong mới code. Khi code: sửa từ `AiO WELCOME` (ID thật `com.aiostudio.*`, đổi cửa sổ Modeless 1020×720 thành Panel dock được), đọc danh sách tool bằng `getExtensions()` thay vì gõ cứng, dọn 2 thư mục `AiO WELCOME` + `AiO WELCOME Page` (luật 4h), sửa cổng debug 8095 trùng `xem-bo.mjs`. ☠️ Hub đọc phiên bản từ manifest → Podcast sẽ hiện **0.1.0** cho tới khi bump |
