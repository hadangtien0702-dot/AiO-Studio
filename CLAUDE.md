...
| 13 | **Video Download** | **0.2.0** | `com.aiostudio.videodownload` · **8098** | 🟢 Đã cài máy công ty 21/09 | Dán link → tải (yt-dlp + QuickJS 2 MB + FFmpeg LGPL) → tự vào bin. 21/09: giao diện mới (hướng A anh chốt), sửa "Mở thư mục" (gốc: explorer bị `windowsHide` → 9 cửa sổ ẩn 1,67 GB), "Đã vào project" thành nhãn sống theo project đang mở, thư mục mặc định `<project>\AiO Studio Download`. Đo trên Premiere thật bằng project thử riêng. Chưa qua bài thật của anh. Chi tiết: `CLAUDE.md` + `PROGRESS.md` trong thư mục nó |
