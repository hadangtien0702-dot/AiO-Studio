...
export interface KetQuaNghe {
  ...
  tu: TuTinCay[]
  /**
   * Ma ngon ngu Whisper NHAN RA (`result.language` trong JSON): vi, en, ja...
   * Ban 2.3.0 chuyen sang `-l auto` nen day la thu duy nhat cho biet no da
   * nghe ra thu tieng gi — va luat cat dong phu thuoc vao no (CJK 32 don vi,
   * con lai 42). Rong = khong doc duoc, ben goi coi nhu nhom khac CJK.
   */
  ngonNgu?: string
}
...
export async function nghe(
  ...
  await execFileAsync(
    boMay.exe,
    // ☠️ `-l auto` — anh Tiến chốt 30/07: *"thêm vài ngôn ngữ phổ thông trên
    // thế giới"*, và chọn để Whisper tự nhận thay vì bày thanh chọn.
    //
    // Whisper trả lại mã ngôn ngữ nó nhận ra trong `result.language`, nên panel
    // biết được sau khi nghe xong — kịp để chọn luật cắt dòng trước khi sinh
    // `.srt` (CJK 32 đơn vị, còn lại 42; xem `gioiHanTheoKhung` trong srt.ts).
    //
    // ⚠️ PHẢI NÓI RÕ GIỚI HẠN, đừng để ai tưởng là đã hỗ trợ đầy đủ: ba tham số
    // dưới đây **đo riêng trên giọng Việt** và đang dùng chung cho mọi thứ tiếng
  ...
  try {
    ...
    throw new Error(dich('Bộ nghe hiểu chạy xong nhưng không thấy file kết quả:\n') + jsonPath)
  }
  try {
...
/**
 * ☠️ TĂNG LÊN 2 NGÀY 30/07 — bắt buộc, không được để 1.
 *
 * Bộ đệm sinh trước hôm nay được chép bằng `-l vi` (khoá cứng tiếng Việt). Bản
 * 2.3.0 chuyển sang `-l auto` và **`KetQuaNghe` có thêm trường `ngonNgu`**, thứ
 * quyết định luật cắt dòng.
 *
 * Đệm cũ không có trường đó → đọc lên thì `ngonNgu` là `undefined` → panel coi
 * như nhóm "khác CJK". Với video tiếng Việt thì vẫn đúng, nhưng với video tiếng
 * Nhật đã chép trước hôm nay thì **cắt sai luật mà không báo gì** — đúng kiểu
 * lỗi tệ nhất: hỏng âm thầm.
 *
 * Tăng phiên bản là mọi đệm cũ tự hết hiệu lực, phải nghe lại một lần. Đắt một
 * lần, nhưng chắc.
 */
  ...
  try {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        moHinh: maMoHinh,
        ...vt,
        // Ngon ngu PHAI luu theo: no quyet dinh luat cat dong, ma lan sau doc
        // dem thi khong con file JSON cua Whisper de doc lai.
        ngonNgu: ket.ngonNgu ?? '',
        cau: ket.cau,
        tu: ket.tu,
      }),
...
export function docDem(mediaPath: string, maMoHinh: string): KetQuaNghe | null {
  try {
    ...
    return {
      cau: j.cau as Cau[],
      tu: (j.tu ?? []) as TuTinCay[],
      ngonNgu: typeof j.ngonNgu === 'string' ? j.ngonNgu : '',
    }
  } catch {
    return null
  }
...
export function docJson(noiDung: string): KetQuaNghe {
  ...
  const cau: Cau[] = []
  const tu: TuTinCay[] = []
  // Whisper tra ve ngon ngu no nhan ra o `result.language`.
