...
| 12 | **AiO Shot & Save** | **0.3.4** | app Electron độc lập (KHÔNG CEP — version từ `package.json`, không có manifest) | — | ✅ **Anh Tiến đã test OK (25/08)** | Chụp vùng (Alt+`, đổi được) + khay ảnh + ghim sticky + vẽ khung/mũi tên 7 màu + Ctrl+C copy + kéo-thả file ra app khác + chụp VẮT NGANG 2 màn + song ngữ VI/EN. **26/08:** chụp vắt 2 màn ghép PIXEL VẬT LÝ (đúng mọi scale — máy anh 150%+125%), hết ảnh tối (content-protection), chọn JPEG/PNG + chất lượng (thấp 125KB/cao 171KB/siêu 221KB), khay hiện dung lượng, khay DỌC (option), run-log luôn bật. **Bộ cài .exe ĐÃ CÓ 26/08** (`Release/2026-08-26-shotandsave-0.3.4/`, NSIS 99MB, đã cài thử chạy thật trên máy công ty; còn [CHỜ]: máy sạch + ký số/SmartScreen) |
