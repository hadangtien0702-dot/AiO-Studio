 ...
 *
 * ☠️ HẰNG TẦNG MODULE — KHÔNG bọc `dich()` trong mảng này (chạy lúc import,
 * trước khi React gắn bảng chữ). Bọc ở CHỖ VẼ RA: `{dich(label)}` dưới kia.
 * `Video` · `Mogrt` · `Preset` giống nhau ở hai thứ tiếng nên không có trong
 * `chu.ts` — không có trong bảng thì `dich()` trả lại nguyên văn.
 */
const TYPE_ITEMS: {
  key: AssetFilter
  label: string
                    ...
                    subs.map((sf) => {
                      ...
                      return (
                        <div
                           ...
                           */
                          /*
                           * ☠️ `label` là HẰNG TẦNG MODULE (tiếng Việt) — phải
                           * `dich(label)` rồi mới `.toLowerCase()`, chứ không
                           * hạ chữ thường trên chuỗi tiếng Việt.
                           */
                          title={
                        ...
                        >
                          ...
                          <button
                            ...
                            title={dich('Quét lại riêng thư mục {ten}').replace('{ten}', sf.name)}
