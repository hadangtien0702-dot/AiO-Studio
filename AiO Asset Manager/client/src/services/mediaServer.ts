  ...
  return new Promise((resolve) => {
    const server = http.createServer((request: any, response: any) => {
      try {
        ...
        // [0.10.0] File DẪN XUẤT (thumbs/proxies trong <userData>/AiOStudio):
        ...
        try {
          const dataRoot = pathMod
            .join(userDataPath() || '', 'AiOStudio')
            .toLowerCase()
          if (dataRoot.length > 'AiOStudio'.length && filePath.toLowerCase().startsWith(dataRoot)) {
