...
- `client/index.html`: `<title>` van con la "AiO Studio — Asset Manager" (di
  san tu hoi tach panel 29/07). Nguoi dung khong thay trong Premiere nhung
  THAY khi mo ban xem ca bo (design-system/xem-bo.html) hay re chuot len tab
  -> voi san pham BAN RA day la kieu loi lam mat tin. Nay: "Power Bins".
- Cai lai ban moi: truoc do ban cai la **v1.3.2** trong khi nguon da len
  **2.0.0** tu dot design-system 29/07 (dist + host van KHOP hash — chi so
  version chua duoc cai). Nay manifest nguon = cai = 2.0.0.
  ⚠️ So version chi co hieu luc sau khi TAT HAN Premiere mo lai (CEP doc
  manifest luc khoi dong); noi dung panel thi da moi ngay sau khi reload.
...
Rieng panel nay: y het Asset Manager (dung chung ~90% ma) - thanh tren truoc
khoa cung `height: 44px`, nen `--bg-2`. Nay dung token.
...
- Extension id com.aiostudio.powerbin, cong debug 8090
- Du an MOI, tach khoi AiO Editing ngay 29/07
- KHO DU LIEU RIENG: %APPDATA% AiOPowerBins library.json.
  Khong bao gio dung toi kho cua Asset Manager - da kiem bang SHA-256: tao brand
  trong panel nay thi file cua Asset Manager KHONG DOI mot byte.
- MVP "mo project KHAC van thay nguyen" DA DAT ngay 29/07. Do tren Untitled.prproj
  moi tao: brand va khay va asset con nguyen, anh xem truoc tai duoc that
  (320x1800, khong phai anh vo), bam Import chen vao timeline chay (V1 tu 0 len
  1 clip). File kho SHA-256 khong doi tuc panel chi DOC.
- Hai duong dua asset vao khay: nut "Them tu timeline" va keo tha tu Explorer.
  Duong thu ba (Import tu luoi Asset Manager) MAT theo thiet ke khi tach kho.
- FFmpeg da chuyen sang LGPL: proxy dung libopenh264
- Dinh khoang 90% ma voi Asset Manager - sua ben do thi NHO CHEP SANG DAY

KE TIEP:
1. [CHO] Bo cai GHEP 4 panel thanh mot
2. [CHO] Chu ky so thuong mai
3. [CHO] Bo cai nhe lai (.zxp 91,5 MB sau khi doi FFmpeg LGPL)

Doc PIPELINE.md o thu muc cha de biet con thieu gi de ban duoc.

---

## [2.0.0-dev.6] - 2026-07-29 16:08 (UTC+7)

### Verified - [HOAN TAT] MVP "MO PROJECT KHAC VAN THAY NGUYEN" - DA DAT

Boi canh: day la dong DUY NHAT trong bang MVP con ghi "chua tung chay phep do
nay", va no chinh la cau chuyen ban hang cua tinh nang (Power Bin = cai ba lo
deo tren lung, di phong nao cung mang theo).

Cach do - phai dung, khong duoc an gian:
KHONG dung lai 	est 2.prproj hay ban auto-save cua no. Gia su du lieu co bi
luu trong project di nua thi ban sao cung mang theo -> nhin vao van thay du,
phep thu vo nghia. PHAI la project chua tung biet Power Bin la gi.
-> Chu du an tao project MOI TOANH (File > New > Project).

### Do duoc
- Project dang mo : Untitled.prproj tai AiO Studio\file pr for test\
  Sequence 01 co 0 clip, ca project chi co 1 item (chinh cai sequence)
- Menu trai       : Brand Kit - Kenh Test 1 - Logo 1  -> CON NGUYEN
- Luoi            : 1 the _Glitch After Noise 1.mp4
- Anh xem truoc   : the IMG, complete=true, 
aturalWidth=320 (320x1800)
  -> TAI DUOC THAT, khong phai anh vo. Day la cho HON Power Bin goc cua Adobe:
     ban do chi cho cai ten file.
- File kho tren dia: SHA-256 truoc 555E347A6D5D3B4233A23E20
                     SHA-256 sau  555E347A6D5D3B4233A23E20  -> KHONG DOI
                     gio sua van 14:49:44 -> panel chi DOC, khong ghi bua
- Dung duoc ngay  : bam Import -> V:0,0,0 thanh V:1,0,0
                     sequence end tu 0 -> 1.260.755.496.000 tick = 4,96 giay

### Ket luan
Vong khep kin: ghim o project nay -> mo project khac van con -> chen thang vao
timeline duoc ngay. MVP nay HET NO.
...
### Added - [HOAN TAT] Du an MOI, tach ra tu AiO Editing

Boi canh: Power Bins tro thanh san pham ban rieng, khong con la mot the trong
panel Asset Manager.

Thay doi:
- ID moi com.aiostudio.powerbin - cong debug 8090 - ten panel "AiO Studio - Power Bins"
- KHO DU LIEU RIENG: %APPDATA%\AiOPowerBins (truoc la dung chung %APPDATA%\AiOStudio)
  Chu du an chot: nguoi dung tu dua asset vao bang nut "Them tu timeline" va keo tha
  tu Explorer. Panel nay TUYET DOI khong doc/ghi library.json cua Asset Manager.
- Mo panel la vao THANG cay Brand -> Khay (activeMasterTab khoi tao 'powerbin')
- Bo man hinh chao va nut "quay lai man hinh chon"

### Chua lam (co y)
- KHONG viet buoc chuyen du lieu tu AiOStudio sang AiOPowerBins. Do that
  2026-07-29: kho cu chi co 1 brand + 2 khay + 0 asset trong khay -> tao lai bang
  tay mat chua toi mot phut, khong bo cong viet migrate.
- Duong dua asset vao thu 3 ("Import tu luoi Asset Manager") MAT theo thiet ke,
  vi hai panel khong con chung kho. Con lai 2 duong.

### File anh huong
client/src/App.tsx - client/src/state/store.ts - client/src/components/Toolbar.tsx
client/src/services/library.ts - cachePaths.ts - mediaServer.ts (doi thu muc du lieu)
CSXS/manifest.xml - .debug - scripts/*.ps1
